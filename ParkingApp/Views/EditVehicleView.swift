...
struct EditVehicleView: View {
    ...
    private func updateVehicle() {
        ...
        // Create and finalize the updated copy BEFORE entering concurrent code
        var updated = vehicle
        updated.vehicleNumber = vehicleNumber.uppercased()
        updated.model = model
        updated.manufacturer = manufacturer.isEmpty ? nil : manufacturer
        updated.color = color.isEmpty ? nil : color
        
        // Immutable capture to satisfy concurrency rules
        let vehicleToUpdate = updated
        
        Task { [vehicleToUpdate] in
            do {
                try await FirestoreManager.shared.updateVehicle(vehicleToUpdate)
                await MainActor.run {
                    isLoading = false
                    showSuccess = true
                }
